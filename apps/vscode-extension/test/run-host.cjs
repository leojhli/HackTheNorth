...
async function main(){
  ...
  const server=spawn(python,['-m','tests.e2e_server'],{cwd:root,env:{...process.env,BEPROGRAM_E2E_PORT:'8011'},windowsHide:true,stdio:['ignore','pipe','pipe']});
  ...
  try{
    ...
    const executable=process.env.BEPROGRAM_VSCODE_EXECUTABLE||path.join(process.env.LOCALAPPDATA||'','Programs','Microsoft VS Code','Code.exe');
    const env={...process.env,BEPROGRAM_TEST_RUN:run};delete env.ELECTRON_RUN_AS_NODE;
    code=spawn(executable,[workspace,'--new-window','--skip-welcome','--skip-release-notes','--disable-workspace-trust','--disable-extensions','--disable-updates','--disable-gpu',
      ...
      '--extensionDevelopmentPath='+(process.env.BEPROGRAM_EXTENSION_PATH||path.join(root,'apps/vscode-extension')),'--extensionTestsPath='+path.join(__dirname,'host.cjs')],{env,windowsHide:true,stdio:['ignore','pipe','pipe']});
