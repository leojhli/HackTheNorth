...
function activate(context) {
  ...
  status.command='beprogram.open';status.text='BeProgram: not started';status.show();
  const controller=new Controller(vscode,context,state=>{
    status.text=!state.session?'BeProgram: not started':state.busy?'BeProgram: working':state.gate?.available?'BeProgram: AI available':'BeProgram: checkpoint required';
  ...
  const provider={
    resolveWebviewView(resolved) {
      ...
      if(!fs.existsSync(path.join(context.extensionPath,'media','webview.js'))) {
        view.webview.html='<html><body><p>Build the BeProgram sidebar first: run scripts/build-extension.ps1 from the repository root.</p></body></html>';return;
      }
  ...
  context.subscriptions.push(status,vscode.window.registerWebviewViewProvider('beprogram.checkpoint',provider,{webviewOptions:{retainContextWhenHidden:false}}));
  const focus=()=>vscode.commands.executeCommand('beprogram.checkpoint.focus');
  ...
  context.subscriptions.push(vscode.commands.registerCommand('beprogram.open',focus));
  command('beprogram.connect','connect');command('beprogram.capture','capture');command('beprogram.ask','ask');
  command('beprogram.stop','end');command('beprogram.refresh','refresh');command('beprogram.disconnect','disconnect');command('beprogram.history','history');
  context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(event=>{
    if(event.affectsConfiguration('beprogram.apiUrl')) {
  ...
  context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc=>{
    ...
    saveTimer=setTimeout(async()=>{
      ...
      const choice=await vscode.window.showInformationMessage('Saved changes are ready for a BeProgram checkpoint preview.','Review changes');
      if(choice)void vscode.commands.executeCommand('beprogram.capture');
    },5000);
  }));
