...
export default function LiveApp() {
  ...
  const [dark, setDark] = useState((localStorage.getItem('codeproof-theme') || localStorage.getItem('beprogram-theme')) !== 'light')
  ...
  const [aiContext, setAiContext] = useState<{files:string[];captured_at:number|null;partial:boolean}|null>(null)
  ...
  useEffect(() => { localStorage.setItem('codeproof-theme', dark ? 'dark' : 'light'); localStorage.removeItem('beprogram-theme') }, [dark])
  useEffect(() => {
    let unsubscribe: (() => void) | undefined
    api<Config>('/v1/config').then(c => {
      setConfig(c)
      if (c.auth_mode === 'local') { const token = sessionStorage.getItem('codeproof-local-token') || sessionStorage.getItem('beprogram-local-token'); if (token) { sessionStorage.setItem('codeproof-local-token',token);sessionStorage.removeItem('beprogram-local-token');setToken(token);setAuthReady(true) } }
  ...
  async function refresh(open = false) {
    ...
    setProjects(ps); setHistory(hs); setProjectsLoaded(true)
  ...
  async function deleteManagedProject(target: Project) {
    let deleted = false
    await run(async () => {
      await api(`/v1/projects/${target.id}`, 'DELETE')
      deleted = true
      setProjects(prev => prev.filter(p => p.id !== target.id))
      setHistory(prev => prev.filter(c => c.project_id !== target.id))
      if (project?.id === target.id) setProject(null)
      if (session?.project_id === target.id) {
        setSession(null); setGate(null); setCp(null)
        setS(initialExtensionState); answerKey.current = null; askKey.current = null
        setPrompt(''); setAiResponse(''); setAiContext(null)
      }
      if (cp?.project_id === target.id) setCp(null)
      setSelectedEvidence(null)
      setNotice(`Deleted ${target.name} and its CodeProof history. Your code files were kept.`)
    })
    return deleted
  }
  const a: ExtActions = {
  ...
  return <ProductContext.Provider value={productData(project,session,cp,history,config)}><div className={dark ? 'dark':'light'}><div className="flex min-h-screen flex-col bg-canvas text-primary">
    <header className="sticky top-0 z-40 border-b border-subtle bg-panel/85 px-4 py-2.5 backdrop-blur"><div className="mx-auto flex max-w-[1440px] flex-wrap items-center gap-x-4 gap-y-2">
      <Wordmark size="sm"/><nav aria-label="Main navigation" className="flex flex-wrap items-center gap-1 rounded-full bg-canvas p-0.5">{[['/','Checkpoint'],['/projects','Projects'],['/history','Website'],['/settings','Settings'],...(config?.capabilities.receipts ? [['/verify','Verifier']] : [])].map(([url,label]) => <button key={url} onClick={()=>navigate(url)} className={`rounded-full px-3 py-1 text-[12px] ${route===url?'bg-action text-on-action':'text-secondary hover:text-primary'}`}>{label}</button>)}</nav>
      <div className="ml-auto flex items-center gap-2"><button aria-label="Refresh saved state" onClick={()=>void run(()=>refresh(true))} disabled={busy || !authReady}><Refresh size={15}/></button><button aria-label="Toggle theme" onClick={()=>setDark(!dark)}>{dark?<Sun size={15}/>:<Moon size={15}/>}</button>{authReady && <Button variant="text" onClick={()=>{void supabase.current?.auth.signOut(); sessionStorage.removeItem('codeproof-local-token');sessionStorage.removeItem('beprogram-local-token');setToken(''); setAuthReady(false); setHistory([]); setProjects([]); setProject(null); setGate(null); setSelectedEvidence(null); setDialog(null); setCp(null); setSession(null)}}>Sign out</Button>}</div>
    </div></header>
    <main className="flex-1 p-3 sm:p-5"><div className="mx-auto max-w-[1200px] space-y-4">
      ...
      {route==='/verify' ? (config?.capabilities.receipts ? <VerifierPage onBack={()=>navigate('/history')}/> : <InlineNotice kind="info">Receipts are disabled for the core demo. <button className="underline" onClick={()=>navigate('/history')}>Return to learning history</button></InlineNotice>) : !authReady ? <SignIn config={config} client={supabase.current} onLocal={token=>{sessionStorage.setItem('codeproof-local-token',token);sessionStorage.removeItem('beprogram-local-token');setToken(token);setAuthReady(true)}} onError={setError}/> : <>
        ...
        {route==='/projects' ? <ProjectsPage projects={projects} activeId={session?.project_id} busy={busy || !projectsLoaded} onCreate={()=>{setProjectName('');setScope('src');setExclusions('');setDialog('project')}} onDelete={deleteManagedProject}/> : route.startsWith('/history') || route==='/settings' ? <LiveWebsite route={route} navigate={navigate} project={project} history={history} config={config} busy={busy} refresh={()=>void run(()=>refresh(false))} onCheckpoint={checkpoint=>{showCheckpoint(checkpoint,gate || undefined);navigate('/')}} onReceipt={checkpoint=>{setSelectedEvidence(checkpoint);setDialog('receipt')}} onImport={()=>setDialog('pr')} onSummary={checkpoint=>{setSelectedEvidence(checkpoint);setDialog('summary')}} onDelete={()=>{if(project) void deleteManagedProject(project).then(deleted=>{if(deleted)navigate('/projects')})}}/> : <>
          ...
          <p className="mx-auto max-w-[860px] text-[12px] leading-5 text-secondary">Managed Ask AI is controlled by CodeProof. Other assistants remain available. The website assesses approved uploaded changes; use the companion for saved Git changes. Unsaved buffers and unsubmitted edits are outside this checkpoint.</p>
    ...
    {dialog==='project' && <Modal onClose={()=>{if(!busy)setDialog(null)}}><div className="space-y-4"><h2 className="text-[18px] font-semibold">Start with an approved project</h2><Field label="Project name" value={projectName} onChange={setProjectName}/><Field label="Included paths (comma separated)" value={scope} onChange={setScope}/><Field label="Excluded paths (comma separated)" value={exclusions} onChange={setExclusions}/><InlineNotice kind="info">Only saved JavaScript/TypeScript in these paths is eligible. Secrets, ignored files and generated output are excluded before upload by the companion.</InlineNotice>{error&&<div role="alert"><InlineNotice kind="error">{error}</InlineNotice></div>}<Button loading={busy} disabled={!projectName.trim()||!scope.trim()} onClick={()=>void run(async()=>{const p=await api<Project>('/v1/projects','POST',{name:projectName.trim(),scope:scope.split(',').map(s=>s.trim()).filter(Boolean),exclusions:exclusions.split(',').map(s=>s.trim()).filter(Boolean)});setProjects(prev=>[...prev,p]);if(!session){setProject(p);patch({phase:'scope'})}setNotice(`Created ${p.name}. Open its folder in VS Code and select this project in CodeProof.`);setDialog(null)})}>Create project</Button></div></Modal>}
    {dialog==='capture' && <Modal onClose={()=>setDialog(null)}><div className="space-y-4"><h2 className="text-[18px] font-semibold">Review saved changes</h2><p className="text-[13px] text-secondary">Preview the companion capture JSON, then explicitly send the approved files to your local CodeProof model. This does not change Git staging.</p><label className="block text-[13px]">Load capture JSON<input className="mt-2 block max-w-full" type="file" accept="application/json,.json" onChange={async e=>{const file=e.target.files?.[0];if(file && file.size<2_000_000)setCaptureJson(await file.text());else setError('Choose a JSON file under 2 MB.')}}/></label><TextArea label="Approved capture JSON" value={captureJson} onChange={e=>setCaptureJson(e.target.value)} className="min-h-[220px] font-mono"/><p className="text-[12px] text-secondary">Shape: files with path, before, after; provenance. Use the local capture companion for filtered saved snapshots.</p><Button loading={busy} disabled={!captureJson.trim()||!session} onClick={()=>void run(async()=>{const input=JSON.parse(captureJson);if(input.blocked?.length)throw new Error('The capture contains blocked files. Narrow the scope first.');patch({phase:'analyzing'});try{const result=await api(`/v1/sessions/${session!.id}/changes`,'POST',{files:input.files,provenance:input.provenance||'unknown',pr_import_id:input.pr_import_id||null,idempotency_key:input.idempotency_key||crypto.randomUUID()});setDialog(null);setNotice(result.status==='skipped'?result.reason || result.checkpoint?.question?.reason:'');await refresh(true)}catch(e){patch({phase:'error'});await refresh(true);throw e}})}>Approve capture and generate question</Button></div></Modal>}
    {dialog==='ask' && <Modal onClose={()=>setDialog(null)}><div className="space-y-4"><h2 className="text-[18px] font-semibold">Managed Ask AI</h2>{!gate?.available && <InlineNotice kind="pending">A checkpoint is unresolved. Complete it before your next managed request.</InlineNotice>}<TextArea label="Coding request" value={prompt} onChange={e=>setPrompt(e.target.value)} className="min-h-[150px]"/><p className="text-[12px] text-secondary">Uses the latest approved saved code excerpts in this project. Unsaved edits and unrelated files are excluded. CodeProof does not edit your files.</p><Button loading={busy} disabled={!gate?.available||prompt.trim().length<3} onClick={()=>void run(async()=>{if(askKey.current?.text!==prompt)askKey.current={text:prompt,key:crypto.randomUUID()};try{const result=await api(`/v1/sessions/${session!.id}/ask`,'POST',{prompt,idempotency_key:askKey.current.key});setAiResponse(result.text);setAiContext(result.context||null)}catch(e){if((e as {code?:string}).code==='ask_context_changed')askKey.current=null;throw e}})}>Send AI request</Button>{aiResponse&&<p className="text-[12px] text-secondary">AI-generated suggestion. Not executed or verified; it may be incorrect.</p>}{aiResponse&&<InlineNotice kind="info">{aiContext?.files.length?`Approved saved excerpts: ${aiContext.files.join(', ')}. Captured ${new Date(aiContext.captured_at!*1000).toLocaleString()}.${aiContext.partial?' Partial context.':''}`:'No approved code excerpts were available. This response is general advice.'}</InlineNotice>}{aiResponse && <pre className="max-h-[350px] overflow-auto whitespace-pre-wrap rounded border border-subtle bg-canvas p-3 text-[13px]">{aiResponse}</pre>}</div></Modal>}
