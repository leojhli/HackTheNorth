...
export default function LiveApp() {
  ...
  async function refresh(open = false) {
    const [ps, sessions, hs] = await Promise.all([api<Project[]>('/v1/projects'), api<Session[]>('/v1/sessions'), api<Checkpoint[]>('/v1/history')])
  ...
  return <ProductContext.Provider value={productData(project,session,cp,history,config)}><div className={dark ? 'dark':'light'}><div className="flex min-h-screen flex-col bg-canvas text-primary">
    <header className="sticky top-0 z-40 border-b border-subtle bg-panel/85 px-4 py-2.5 backdrop-blur"><div className="mx-auto flex max-w-[1440px] flex-wrap items-center gap-x-4 gap-y-2">
      <Wordmark size="sm"/><nav aria-label="Main navigation" className="flex items-center gap-1 rounded-full bg-canvas p-0.5">{[['/','Checkpoint'],['/history','Website'],['/settings','Settings'],['/verify','Verifier']].map(([url,label]) => <button key={url} onClick={()=>navigate(url)} className={`rounded-full px-3 py-1 text-[12px] ${route===url?'bg-action text-on-action':'text-secondary hover:text-primary'}`}>{label}</button>)}</nav>
    ...
    <main className="flex-1 p-3 sm:p-5"><div className="mx-auto max-w-[1200px] space-y-4">
      ...
      {route==='/verify' ? <VerifierPage onBack={()=>navigate('/history')}/> : !authReady ? <SignIn config={config} client={supabase.current} onLocal={token=>{sessionStorage.setItem('beprogram-local-token',token);setToken(token);setAuthReady(true)}} onError={setError}/> : <>
        {config && !config.capabilities.assessment && <InlineNotice kind="info">OpenAI is not configured. Project setup and saved history work; assessment and managed Ask AI require a server API key.</InlineNotice>}
