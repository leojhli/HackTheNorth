...
test('sidebar UI: saved capture, draft restoration, follow-up, give up and next managed request',async({page,request,baseURL})=>{
  ...
  try{
    ...
    await expect(page.getByLabel('Patch companion')).toHaveAttribute('data-mood','idle')
    expect(await page.getByLabel('Patch companion').locator('.bp-patch-character').evaluate(node=>getComputedStyle(node).animationName)).not.toBe('none')
    await page.emulateMedia({reducedMotion:'reduce'})
    expect(await page.getByLabel('Patch companion').locator('.bp-patch-character').evaluate(node=>getComputedStyle(node).animationName)).toBe('none')
    ...
    await expect(page.getByLabel('Patch companion')).toHaveAttribute('data-mood','focused')
    ...
    await expect(page.getByRole('heading',{name:'Question completed',exact:true})).toBeVisible()
    await expect(page.getByRole('region',{name:'Code explanation'})).toContainText('driver binds email as data')
    await expect(page.getByLabel('Your follow-up answer')).toHaveCount(0)
    await expect(page.getByRole('button',{name:'Give up and explain',exact:true})).toHaveCount(0)
    await expect(page.getByRole('button',{name:'Try a practice question',exact:true})).toHaveCount(0)
    expect(controller.state.checkpoint.status).toBe('given_up')
    expect(controller.state.checkpoint.passed_at).toBeNull()
    expect(controller.state.gate.available).toBe(true)
    await page.reload()
    await expect(page.getByRole('heading',{name:'Question completed',exact:true})).toBeVisible()
    await expect(page.getByRole('button',{name:'Managed Ask AI',exact:true})).toBeEnabled()
    await page.screenshot({path:'test-results/vscode-sidebar-given-up.png',fullPage:true,animations:'disabled'})
