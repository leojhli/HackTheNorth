...
def inspect(config, client=None):
    ...
    add('extension_package', Path('artifacts/beprogram-companion.vsix').is_file(), 'Install the VSIX using Extensions: Install from VSIX.')
    ...
    try:
        ...
        data = running.json()
        matches = data.get('integration') == 'beprogram_managed' and data.get('cost_mode') == 'local_only' and data.get('ai', {}).get('model') == config.ollama_model
        ...
        add('backend_connection', False, 'Start ./scripts/run-local.ps1; confirm port 8000 belongs to BeProgram.')
    finally:
        if own_client:
            client.close()
...
def main():
    ...
    if args.json:
        ...
        print('Ready for BeProgram.' if report['ready'] else 'Resolve the FIX items above, then run this command again.')
