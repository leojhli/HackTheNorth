...
| SEARCH-01 / LEARN-01 | Not implemented | P2 deferred; no need established for extra infrastructure |
...
Latest prior baseline: 60 backend tests pass; three app browser scenarios, seven extension unit tests and six host groups passed in earlier milestones. That historical VSIX was 0.3.0; current release is 0.4.0. Test doubles prove API/state/UI behavior, not model judgment. A real beginner demo and six rejection checks passed after quote recovery. A prior 20-case calibration run matched 18/20 agent labels with zero accepted false passes but two complete answers blocked and p95 75.765 seconds; it predates the quote fix and is not current accuracy or a human review.
